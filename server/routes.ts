...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.delete("/api/whatsapp/instances/:id", async (req, res) => {
    try {
      ...
      console.log(`[WhatsApp] Instance deleted: ${instance.name} (ID: ${id})`);
      res.status(204).send();
    } catch (error) {
  ...
  app.post("/api/whatsapp/instances/:id/connect", async (req, res) => {
    try {
      ...
      const id = parseInt(req.params.id);
      // Redirect to Baileys connect - keep instance record updated
      await baileysManager.connect(authUserId);
  ...
  app.post("/api/billing/send", async (req, res) => {
    try {
      ...
      for (const clientId of clientIds) {
        ...
        const clientNumber = String(client.clientNumber ?? "").padStart(4, "0");
        let message = template.content
          .replace(/{nome}/g, client.name)
          .replace(/{numero_cliente}/g, clientNumber)
