...
process.on('unhandledRejection', (reason) => {
  console.error('[UnhandledRejection] Caught unhandled promise rejection (server kept running):', reason);
});

process.on('uncaughtException', (err) => {
  console.error('[UncaughtException] Caught uncaught exception (server kept running):', err);
});
...
(async () => {
  ...
  // Use PORT from environment (Railway) or default to 5000 (Replit/local)
